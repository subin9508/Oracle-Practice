 ...
 */
 ...
 as select * from emp where empno = -1; -- 고유키의 없는 값을 조건으로 설정하면 됨.
 ...
 select * from emp  where sal between (select losal from salgrade where grade = 1) and (select hisal from salgrade where grade = 1);
 
 -- delete 문: 테이블에서 행 삭제(DML)
 -- delete from 테이블 where 조건식; where 생략 가능. (대신 모든 행이 삭제됨)
