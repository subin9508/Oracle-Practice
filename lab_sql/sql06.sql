 ...
 */
 ...
 -- inner join과 outer join의 차이점을 설명하기 위해서 추가하는 데이터.
 ...
 select e.empno, e.ename, e.deptno, d.dname 
 from emp e, dept d
 where e.deptno = d.deptno(+); -- 왼쪽 테이블의 정보는 모두 보여주고, 오른쪽 테이블의 정보는 비어있을 시 null을 추가하여 보여준다고 생각하여 null이 필요한 테이블에 (+)를 붙여준다.
 ...
 -- 집합 연산(합집합( union, 교집합 intersect, 차집합)을 사용.
 select e.empno, e.ename, e.deptno, d.deptno, d.dname
 from emp e, dept d
 where e.deptno = d.deptno(+)
 intersect
 ...
 select e1.empno, e1.ename, e1.mgr as "MGR_NO", e2.ename as "MGR_NAME" -- 별명 붙일 때 as 생략 가능.
 from emp e1
 ...
 from emp e2
    right outer join emp e1 on e1.mgr = e2.empno;
...
-- ex2. 직원 이름, 매니저 이름, 직원의 급여, 급여 등급을 검색.
--      정렬순서: (1)매니저, (2)직원의 급여 등급
...
-----
select d.deptno, d.dname, d.loc, count(*) as "직원수"
from emp e
   right outer join dept d on e.deptno = d.deptno
...
select d.deptno, d.dname, count(*) as "직원수", min(sal), max(sal)
from emp e
 inner join dept d on e.deptno = d.deptno
...
select e1.deptno, d.dname, e1.empno, e1.ename, e1.mgr as "MGR_NO", e2.ename as "MGR_NAME", e1.sal, s.grade
from emp e1 
 inner join dept d on e1.deptno = d.deptno
 inner join emp e2 on e1.mgr = e2.empno
 inner join salgrade s on e1.sal between s.losal and s.hisal 
 where e1.sal >= '3000'
 order by e1.deptno asc, e1.empno asc; 
